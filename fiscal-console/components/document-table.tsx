  ...
  return (
    <div className="table-wrap">
      ...
      <div className="table-mobile">{rows.map((row) => (
        <Link href={`/documentos/${row.id}`} key={row.id}><article><div className="document-cell"><span className="document-code">{row.kind}</span><span><strong>{row.label}</strong><small>Folio {row.folio} · {row.issuedAt}</small></span></div><strong>{row.amount ? clp(row.amount) : "—"}</strong><StatusPill status={row.status}>{row.statusLabel}</StatusPill><ArrowUpRight size={18} /></article></Link>
      ))}</div>
    </div>
  );
}
