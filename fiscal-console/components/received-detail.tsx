...
export function ReceivedDetail() {
  ...
  async function confirm() {
    if (!decision) return;
    const response = await fetch("/api/demo/activities?area=received_decision", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ document: "33-7841", decision, code: selected?.code }) });
