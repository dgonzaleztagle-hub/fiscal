...
export async function Dashboard() {
  const { rows, source } = await fiscalDocuments();
  const documentedSales = rows.reduce((sum, row) => sum + row.amount, 0);
  const accepted = rows.filter(row => row.status === "accepted").length;
  const pending = rows.filter(row => row.status === "submitted" || row.status === "attention").length;
  return (
    <div className="page dashboard-page">
      ...
      <section className="metrics" aria-label="Resumen de hoy">
        <article><span className="metric-icon mint"><Landmark size={19} /></span><p>Ventas documentadas</p><strong>{clp(documentedSales)}</strong><small className="positive"><TrendingUp size={13} /> {source === "sandbox" ? "Backend sandbox conectado" : "Datos de demostración"}</small></article>
        <article><span className="metric-icon blue"><CheckCircle2 size={19} /></span><p>Documentos aceptados</p><strong>{accepted}</strong><small>de {rows.length} documentos visibles</small></article>
        <article><span className="metric-icon amber"><Clock3 size={19} /></span><p>En proceso SII</p><strong>{rows.filter(row => row.status === "submitted").length}</strong><small>Simulador, sin conexión externa</small></article>
        <article className="attention-card"><span className="metric-icon coral"><AlertTriangle size={19} /></span><p>Requieren atención</p><strong>{pending}</strong><Link href="/envios">Resolver ahora <ArrowRight size={13} /></Link></article>
      </section>
      ...
      <div className="dashboard-grid">
        <section className="panel sales-panel"><div className="panel-heading"><div><p className="eyebrow">Últimos 14 días</p><h2>Ventas documentadas</h2></div><Link className="quiet-button" href="/documentos">Ver detalle</Link></div><div className="sales-total"><strong>{clp(8935720)}</strong><span>+8,2% vs. período anterior</span></div><div className="bar-chart" aria-label="Gráfico de ventas de 14 días">{bars.map((height, index) => <span key={index} style={{ height: `${height}%` }} className={index === bars.length - 1 ? "current" : ""} />)}</div><div className="chart-axis"><span>26 jun</span><span>30 jun</span><span>4 jul</span><span>Hoy</span></div></section>
