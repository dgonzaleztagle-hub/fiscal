...
export function DocumentDetail({ document }: { document: FiscalDocumentDetail }) {
  return <div className="page section-page document-detail-page">
    ...
    <div className={`data-source ${document.source}`}>{document.source === "engine" ? "Motor fiscal conectado · registro inmutable" : document.source === "sandbox" ? "Backend sandbox · documento sintético persistido" : "Datos demostrativos · sin XML real"}{document.warning ? ` · ${document.warning}` : ""}</div>
    <div className="document-detail-grid">
      ...
      <aside className="panel artifact-panel"><p className="eyebrow">Representaciones</p><h2>Archivos tributarios</h2><p>Las descargas pasan por el servidor de la consola; la credencial del motor nunca llega al navegador.</p>{document.source !== "demo" ? <div className="artifact-actions"><a className="secondary-button" href={fiscalArtifactUrl(document.id, "xml")}><FileCode2 size={17} /> Descargar XML <Download size={14} /></a><a className="secondary-button" href={fiscalArtifactUrl(document.id, "pdf")} target="_blank" rel="noreferrer"><FileText size={17} /> Abrir PDF <ExternalLink size={14} /></a>{document.source === "sandbox" && <small>Representaciones sintéticas marcadas como no válidas; jamás se envían al SII.</small>}</div> : <button className="secondary-button" disabled>Conecta el backend sandbox para descargar</button>}</aside>
    </div>
