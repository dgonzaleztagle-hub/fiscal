...
const initialReadiness: Readiness = { completed: 2, total: 5, gates: [
  ...
  { code: "public_https", title: "Consulta pública HTTPS", completed: false, detail: "Requiere despliegue accesible antes de certificar" },
  { code: "real_certificate", title: "Certificado digital real", completed: false, detail: "Pendiente de compra y carga en vault" },
  { code: "real_caf", title: "CAF real de cinco folios", completed: false, detail: "Bloqueado para no iniciar la ventana de 24 horas" },
] };
...
export function CertificationCockpit() {
  ...
  useEffect(() => { Promise.resolve().then(() => setHydrated(true)); fetch("/api/certification").then((r) => r.json()).then(setReady); }, []);
