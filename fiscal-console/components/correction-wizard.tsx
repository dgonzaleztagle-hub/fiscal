...
import { ArrowLeft, ArrowUpDown, Ban, CheckCircle2, FilePenLine, Info, ShieldAlert } from "lucide-react";
...
export function CorrectionWizard() {
  ...
  return <div className="page section-page wizard-page"><Link href="/emitir" className="back-link"><ArrowLeft size={15} /> Volver a tipos de documento</Link><header className="page-header"><div><p className="eyebrow">Asistente legal · Sandbox</p><h1>¿Qué necesitas corregir?</h1><p>Primero define la intención. Completo elegirá el documento y bloqueará combinaciones incompatibles.</p></div><span className="demo-action">Builders 56/61 activos</span></header><div className="correction-layout"><section className="panel correction-choices">{choices.map(({ id, icon: Icon, title, detail }) => <button className={intent === id ? "selected" : ""} key={id} type="button" onClick={() => setIntent(id)}><span><Icon size={19} /></span><div><strong>{title}</strong><p>{detail}</p></div>{intent === id && <CheckCircle2 size={17} />}</button>)}{intent === "amount" && <div className="direction-choice"><button className={direction === "down" ? "active" : ""} type="button" onClick={() => setDirection("down")}>Disminuir monto</button><button className={direction === "up" ? "active" : ""} type="button" onClick={() => setDirection("up")}>Aumentar monto</button></div>}</section><aside className="panel legal-result"><p className="eyebrow">Resultado documental</p><div className="result-code"><span>{result.code}</span><div><strong>{result.title}</strong><p>{result.reference}</p></div></div><ul>{result.rules.map((rule) => <li key={rule}><CheckCircle2 size={14} /> {rule}</li>)}</ul><div className="legal-warning"><ShieldAlert size={18} /><p>{result.warning}</p></div><div className="form-notice"><Info size={16} /><p>Los flujos 56/61 ya generan TED, XML firmado, referencia inmutable y validación XSD. Esta vista aún no consume folios reales.</p></div></aside></div></div>;
}
