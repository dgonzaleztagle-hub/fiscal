...
class ReceivedDecision:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ReceivedDecisionError(
                "La aceptación o recibo no lleva razón de reclamo"
            )
        if self.decision not in {
            ...
            if self.reason is None or not self.reason.strip():
                raise ReceivedDecisionError(
                    "El reclamo requiere una razón comprensible"
                )
            try:
                size = len(self.reason.encode("iso-8859-1"))
            except UnicodeEncodeError as exc:
                raise ReceivedDecisionError(
                    "La razón contiene caracteres no admitidos"
                ) from exc
            if size > 200:
